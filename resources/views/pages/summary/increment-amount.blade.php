...
<div id="increment" class="secondary-default-bg summary-overview">
    <div class="container-fluid">
        <div class="row wrapper-bottom-grey">
            <div class="header col-12">
                <div class="row">@include('templates.nav.nav-red-menu-needs')</div>
            </div>
            <form novalidate action="{{route('validate.summary.increment.amount')}}" method="POST" class="content-needs-grey">
                @csrf
                <div class="top-menu pt-md-0 py-3"></div>
                        ...
                        <div class="row h-100">
                            ...
                            <div class="col-xl-4 col-lg-6 col-md-6 py-lg-5 pt-4 calculatorContent">
                                <div class="row h-sm-100">
                                    <h2 class="display-5 fw-bold lh-sm">I am expecting my income to increase by</h2>
                                    <p class="display-5 fw-bold currencyField">
                                        <span class="text-primary fw-bold border-bottom border-dark border-3">RM<input type="text" name="approximate_increment_amount" class="form-control fw-bold position-relative border-0 d-inline-block w-50 text-primary @error('approximate_increment_amount') is-invalid @enderror" id="approximate_increment_amount" value="{{ $approximateAmount !== null ? number_format(floatval($approximateAmount)) : $approximateAmount }}" required></span>/ month
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <section class="footer fixed-bottom">
                    @if ($errors->has('approximate_increment_amount') || $errors->has('approximate_increment_amount'))
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-12 alert alert-danger d-flex justify-content-center align-items-center py-2 m-0 rounded-0" role="alert">
                                    ...
                                    <div class="text">{{ $errors->first('approximate_increment_amount') }} {{ $errors->first('approximate_increment_amount') }}</div>
                                </div>
                            </div>
                        </div>
                    @endif
...
<div class="modal fade" id="missingYesInputFields" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header px-4 pt-4 justify-content-center">
                <h3 class="modal-title fs-4 text-center" id="missingYesInputFieldsLabel">You're required to select <strong>Yes</strong> before you proceed to this page.</h2>
            </div>
            <div class="modal-body text-dark text-center px-4 pb-4">
                <p>Please click proceed to input the value in previous page first.</p>
            </div>
...
<script>
    var lastPageInput = '{{$selectedExpectingInput}}';
</script>
@endsection