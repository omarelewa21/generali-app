<?php
 /**
 * Template Name: Debt Cancellation Coverage Page
 */
?>
...
@section('title')
<title>Debt Cancellation - Coverage</title>
...
<div id="debt-coverage" class="secondary-default-bg">
    <div class="container-fluid">
        <div class="row wrapper-bottom-grey">
            <div class="header col-12"><div class="row navbar-scroll">@include('templates.nav.nav-red-menu-needs')</div></div>
            <form novalidate action="{{route('validate.debt.cancellation.coverage')}}" method="POST" class="content-needs-grey">
                @csrf
                        ...
                        <div class="row justify-content-center h-100 coverage_slick">
                            @if ($selfData)
                                <div class="h-100 d-flex justify-content-center align-items-center col-3">
                                    <button class="border-0 bg-transparent position-relative choice d-flex justify-content-center h-100 @if($debtCancellationSelectedAvatar === $selfData['full_name']) default @endif" id="{{ $selfData['full_name'] }}" data-avatar="{{ $selfData['full_name'] }}" data-required="">
                                        <div>
                            ...
                            @if ($spouseData)
                                <div class="h-100 d-flex justify-content-center align-items-center col-3">
                                    <button class="border-0 bg-transparent choice h-100 position-relative d-flex justify-content-center @if($debtCancellationSelectedAvatar === $spouseData['full_name']) default @endif" id="{{ $spouseData['full_name'] }}" data-avatar="{{ $spouseData['full_name'] }}" data-required="">
                                        <div>
                            ...
                            @if ($childData)
                                @foreach($childData as $child)
                                    <div class="h-100 d-flex justify-content-center align-items-center col-3">
                                        <button class="border-0 bg-transparent choice h-100 position-relative d-flex justify-content-center @if($debtCancellationSelectedAvatar === $child['full_name']) default @endif" id="{{ $child['full_name'] }}" data-avatar="{{ $child['full_name'] }}" data-required="">
                                            <div>
                ...
                <section class="footer fixed-bottom">
                    @if ($errors->has('debtSelectedAvatarInput'))
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-12 alert alert-danger d-flex justify-content-center align-items-center py-2 m-0 rounded-0" role="alert">
                                    ...
                                    <div class="text">{{ $errors->first('debtSelectedAvatarInput') }}</div>
                                </div>
                            </div>
                        </div>
                    @endif
                        ...
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-12 d-flex gap-2 d-md-block text-end px-4">
                                    <input type="hidden" name="debtSelectedAvatarInput" id="debtSelectedAvatarInput" value="{{$debtCancellationSelectedAvatar}}">
                                    <a href="{{route('debt.cancellation.home')}}" class="btn btn-secondary flex-fill me-md-2 text-uppercase">Back</a>
